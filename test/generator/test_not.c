...
static void generate_test_case_not_reg(FILE *fp, const UnaryOperationInfo *op_info, const RegisterInfo *reg_info)
{
    ...
    put_line_with_tab(fp, "not %s    # test target", reg);
...
static void generate_test_case_not_mem(FILE *fp, const UnaryOperationInfo *op_info, size_t size)
{
    const char *arg1 = get_1st_argument_register(size);
    const char *arg2 = get_2nd_argument_register(size);
    ...
    put_line_with_tab(fp, "not %s [rbp-%lu]    # test target", size_spec, offset);
...
static void generate_all_test_case_not(FILE *fp)
{
    // NOT reg
    for(size_t i = 0; i < REG_LIST_SIZE; i++)
    {
        ...
        uintmax_t operand = 1;
        uintmax_t result = imm_list[convert_size_to_index(reg_info->size)].uint_max_value - operand;
        const UnaryOperationInfo *op_info = &(UnaryOperationInfo){operand, result};
        generate_test_case_not_reg(fp, op_info, reg_info);
        put_line(fp, "");
    }
    ...
    for(size_t j = 0; j < IMM_LIST_SIZE; j++)
    {
        ...
        uintmax_t operand = 1;
        uintmax_t result = imm_info->uint_max_value - operand;
        const UnaryOperationInfo *op_info = &(UnaryOperationInfo){operand, result};
        generate_test_case_not_mem(fp, op_info, imm_info->size);
        put_line(fp, "");
    }
}
...
{
    generate_test("test/test_not.s", STACK_ALIGNMENT, generate_all_test_case_not);
}
