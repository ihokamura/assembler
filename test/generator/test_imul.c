...
static BinaryOperationInfo make_test_data_imul_reg_regmem_imm(intmax_t imm_value)
{
    intmax_t lhs = 1;
    intmax_t rhs = imm_value;
    return make_test_data_imul(lhs, rhs);
}
...
static void generate_test_case_imul_reg_reg_imm(FILE *fp, const BinaryOperationInfo *op_info, const RegisterInfo *reg1_info, const RegisterInfo *reg2_info)
{
    size_t size = reg1_info->size;
    size_t index_list[] = {reg1_info->index, reg2_info->index};
    const char *reg1 = reg1_info->name;
    ...
    const char *work_reg = generate_save_register(fp, index_list, sizeof(index_list) / sizeof(index_list[0]));
    put_line_with_tab(fp, "mov %s, 0x%llx", reg2, op_info->lhs);
    put_line_with_tab(fp, "%s %s, %s, 0x%llx    # test target", op_info->mnemonic, reg1, reg2, op_info->rhs);
    put_line_with_tab(fp, "mov %s, %s", arg2, reg1);
    put_line_with_tab(fp, "mov %s, 0x%llx", arg1, op_info->result);
    generate_restore_register(fp, work_reg);
    put_line_with_tab(fp, "call assert_equal_uint%ld", convert_size_to_bit(size));
...
static void generate_test_case_imul_reg_mem_imm(FILE *fp, const BinaryOperationInfo *op_info, const RegisterInfo *reg_info)
{
    size_t size = reg_info->size;
    size_t index_list[] = {reg_info->index};
    const char *reg = reg_info->name;
    ...
    put_line_with_tab(fp, "%s %s, %s [%s-%lu], 0x%llx    # test target", op_info->mnemonic, reg, size_spec, work_reg, offset, op_info->rhs);
    put_line_with_tab(fp, "mov %s, %s", arg2, reg);
    put_line_with_tab(fp, "mov %s, 0x%llx", arg1, op_info->result);
    generate_restore_register(fp, work_reg);
    put_line_with_tab(fp, "call assert_equal_uint%ld", convert_size_to_bit(size));
...
static void generate_all_test_case_imul(FILE *fp)
{
    ...
    for(size_t i = 0; i < REG_LIST_SIZE; i++)
    {
        ...
        for(size_t j = 0; j < REG_LIST_SIZE; j++)
        {
            ...
            if((reg1_info->size == reg2_info->size) && (reg1_info->size > sizeof(uint8_t)))
            {
                for(size_t k = 0; k < IMM_LIST_SIZE; k++)
                {
                    const ImmediateInfo *imm_info = &imm_list[k];
                    if((reg1_info->size >= imm_info->size) && (imm_info->size <= sizeof(uint32_t)))
                    {
                        BinaryOperationInfo op_info = make_test_data_imul_reg_regmem_imm(imm_info->sint_max_value);
                        generate_test_case_imul_reg_reg_imm(fp, &op_info, reg1_info, reg2_info);
                        put_line(fp, "");
                    }
                }
            }
    ...
    for(size_t i = 0; i < REG_LIST_SIZE; i++)
    {
        ...
        for(size_t j = 0; j < IMM_LIST_SIZE; j++)
        {
            const ImmediateInfo *imm1_info = &imm_list[j];
            if((reg_info->size == imm1_info->size) && (reg_info->size > sizeof(uint8_t)))
            {
                for(size_t k = 0; k < IMM_LIST_SIZE; k++)
                {
                    const ImmediateInfo *imm2_info = &imm_list[k];
                    if((reg_info->size >= imm2_info->size) && (imm2_info->size <= sizeof(uint32_t)))
                    {
                        BinaryOperationInfo op_info = make_test_data_imul_reg_regmem_imm(imm2_info->sint_max_value);
                        generate_test_case_imul_reg_mem_imm(fp, &op_info, reg_info);
                        put_line(fp, "");
                    }
                }
            }
